...
export const en = {
  ...
  "tab-compare": "Compare",
  // Schema comparison (ADR-0148 / ADR-0149). The report is the whole database;
  // the sidebar marks the tables it mentions and jumps into it.
  "compare-left": "This side",
  "compare-right": "Compared with",
  "compare-pick-target": "Pick a connection…",
  "compare-no-target": "No other { $kind } connection to compare with",
  "compare-run": "Compare",
  "compare-running": "Comparing…",
  "compare-summary": "{ $count } tables differ",
  "compare-table-count": "{ $count } differences",
  "compare-only-in": "only in { $name }",
  "compare-no-differences": "No differences in what was compared",
  "compare-scope-note": "Tables, columns, types, nullability, defaults and primary keys were compared. Indexes and constraints were not.",
  "compare-scope-note-fk": "Tables, columns, types, nullability, defaults, primary keys and foreign keys were compared. Indexes and other constraints were not.",
  "compare-scope-note-all": "Tables, columns, types, nullability, defaults, primary keys, foreign keys and indexes were compared. Other constraints were not.",
  "compare-index-columns": "indexed columns",
  "compare-index-unique": "uniqueness",
  "compare-fk-table": "referenced table",
  "compare-fk-columns": "referenced columns",
  "compare-fk-name": "constraint name",
  "compare-intro-title": "Compare two schemas of the same engine.",
  "compare-intro-body": "Both sides are read only. Types are compared exactly as each engine spells them, so a difference here is a difference in the text, not a judgement about whether the two mean the same thing.",
  "compare-field-declaredtype": "type",
  "compare-field-nullable": "nullability",
  "compare-field-default": "default",
  "compare-field-primarykey": "primary key",
  "compare-mark-changed": "differs from the compared connection",
  "compare-mark-one-side": "exists on one side only",
...
const ja: Partial<Record<MessageKey, string>> = {
  ...
  "compare-left": "この接続",
  "compare-right": "比較先",
  "compare-pick-target": "接続を選ぶ…",
  "compare-no-target": "比較できる { $kind } の接続が他にありません",
  "compare-run": "比較する",
  "compare-running": "比較中…",
  "compare-summary": "{ $count } 個のテーブルに差",
  "compare-table-count": "{ $count } 件の差",
  "compare-only-in": "{ $name } のみ",
  "compare-no-differences": "比較した範囲に違いはありません",
  "compare-scope-note": "テーブル・列・型・NULL 可否・既定値・主キーを比較しました。インデックスと制約は比較していません。",
  "compare-scope-note-fk": "テーブル・列・型・NULL 可否・既定値・主キー・外部キーを比較しました。インデックスとその他の制約は比較していません。",
  "compare-scope-note-all": "テーブル・列・型・NULL 可否・既定値・主キー・外部キー・インデックスを比較しました。その他の制約は比較していません。",
  "compare-index-columns": "対象の列",
  "compare-index-unique": "一意性",
  "compare-fk-table": "参照先テーブル",
  "compare-fk-columns": "参照先の列",
  "compare-fk-name": "制約名",
  "compare-intro-title": "同じエンジンの 2 つのスキーマを比較します。",
  "compare-intro-body": "どちらも読み取りのみ。型はエンジンが綴ったまま比較するので、ここでの差は綴りの差であって、2 つが同じ意味かどうかの判断ではありません。",
  "compare-field-declaredtype": "型",
  "compare-field-nullable": "NULL 可否",
  "compare-field-default": "既定値",
  "compare-field-primarykey": "主キー",
  "compare-mark-changed": "比較先と差があります",
  "compare-mark-one-side": "片側にしかありません",
