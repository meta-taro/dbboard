...
database an agent has just set up is usable without asking a person to
retype its path — and only for the kinds that store nothing in the keychain,
a local SQLite/libSQL file and the Firestore emulator (ADR-0134). Everything
that needs a password, a token or a key is refused in the tool's own
description, before the credential is sent rather than after.
`set_connection_mark` and `move_connection` colour and reorder the list
(ADR-0136), which is the wrong shape of work for a person doing it twenty
times and the right shape for an agent doing it once. `export_connections`
seals connections into an encrypted bundle for another machine, mints the
passphrase itself and files it in this machine's keychain rather than
returning it, and is refused entirely until a human has named a directory
for the files in `connections.toml` (ADR-0140).
...
- **Exporting is off until a human names a directory for it**
  (`[mcp_export]` in `connections.toml`), and the passphrase for what it
  seals is generated by dbboard, filed in this machine's keychain, and
  never returned — a tool result is plaintext in an agent's transcript, so
  the bundle and its key must not travel together (ADR-0140).
