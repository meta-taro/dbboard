...
|---|---|---|
| **v0.18** | The measurement nobody has taken | The **cold-start numbers**, which v0.17 reserved and did not carry: they have to be taken on the maintainer's machine against a real connection, and no agent can do that alone — so the slot moved rather than the work being faked ([ADR-0110](decisions.md), [ADR-0122](decisions.md)). Alongside it, whatever the **`history.jsonl` decision** turns out to be ([issue 0033](../.claude/issues/0033-history-jsonl-has-no-writer.md), [ADR-0153](decisions.md)) — restoring the writer or retiring the schema, both of which need `dbboard-web` in the conversation. Neither is new work invented for the slot; both are things this repository already knows it owes. Alongside them, **rolling back is made a supported move** ([ADR-0155](decisions.md)): each build copies `connections.toml` aside on its first run, and the download page lists previous versions so installing a specific one is a path rather than an archaeology exercise. That arrived as a question — *what happens if the new release does not work?* — which the repository had no answer to |
...
- **Right now (2026-08-20)**: `desktop` has shipped Phases 1 through 6 and
  ...
  separate download from the release page. v0.5.1 is a patch on top: it
