...
|---|---|---|
| **v0.18** | Going back, and the first number from inside | **Rolling back is a supported move** ([ADR-0155](decisions.md)): each build copies `connections.toml` aside on its first run, and the download page lists previous versions, because an older build refuses the *whole* file when it meets a connection kind added after it. The **rustls advisory** ([RUSTSEC-2026-0285](https://rustsec.org/advisories/RUSTSEC-2026-0285)) taken while it was open. And the first startup number that could not be taken from outside ([ADR-0156](decisions.md)): the app reports its own **first paint**, which an external poller cannot see — it learns when a window exists and nothing more. **That is the instrument, not the measurement.** The cold-start numbers move to v0.19 rather than this slot shipping without its headline again |
