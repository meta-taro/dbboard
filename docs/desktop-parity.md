...
| JSON export (typed: NULL stays null, documents nest, blobs stay tagged) | ➖ | ✅ | ADR-0146. Desktop **leads** egui: the delimited exports flatten every cell to a string, which is right for Excel and wrong for a program reading the file back. Pure serializer in `$lib/export/json.ts`; the save confirmation names the row count when the grid holds one page of a paged browse (ADR-0145). |
| Saved queries (named statements per connection) | ➖ | ✅ | ADR-0147. Desktop **leads** egui: `saved-queries.toml` in the config dir, sibling to `annotations.toml` — deliberately not the webview `localStorage` the run history uses, which "clear site data" empties and no backup sees. Overwrite needs an explicit confirmation; not an MCP tool. |
| Schema diff between two connections | ➖ | ✅ | ADR-0148 / ADR-0149. Desktop **leads** egui: a third **Compare** tab. Same engine only, refused before either side is dialled; types compared exactly as spelled. The sidebar marks the tables the report mentions and jumps into it, and never filters or replaces it. | ADR-0151 adds foreign keys, matched on their columns rather than their generated names; four adapters answer (Turso included), and the empty state says whether keys were part of the comparison. Indexes are still out (issue 0035).
