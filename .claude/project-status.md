...
- 日付: 2026-09-24 (**本線の保留は解けていた。訂正 3 件。LP の相互紹介が公開された。**

  ### 保留の解除条件は 9 日前に満たされていた

  最優先の据え方は「git-qa で人が署名した記録が残るまで本線を保留」で、
  **その条件は 2026-09-15 に達成されていた** (`runs/20260915-185839`・
  本題と対照が `VERIFIED`・3 件目は意図した `SKIP`・operator `rays`)。

  **私は `runs/20260916-192701` を証拠として参照していた。**それは
  `desktop-0-17.tsv` (v0.17.0 の見た目検証) で、**別のシート**。どちらも実在し
  どちらも署名入りなので、**誤りは「取れているか」ではなく「どれが証拠か」**だった。

  **この 1 件の取り違えが、存在しないブロッカーを 2 つ生んだ**
  —— 「git-qa#13 待ち」と「判定キーを押す人が現地にいない」。
  後者は `next-actions.md` の 1 番に置くところまで行った。
  **run ディレクトリは日付で並ぶので、近い日付の別シートと取り違えても外からは矛盾しない。**
  `run.json` の `sheet.title` を読むまで区別が付かない。

  なお **git-qa#13 (ライブビューが読めない) も修正済み** —— ライブ映像の拡大が入り、
  **寄る先は AI が指した場所**になった。待っていた理由は両方消えている。

  ### 回線 —— 断じたのが早すぎた

  午前中、GitHub への到達が断続的に落ちた。traceroute で**手元のルーター自身が `!N`**
  を返しており、全 IP・443 も 22 も全滅、`api.anthropic.com` も 0.26 秒で即断。
  **9/23 に「回線の問題ではない、IP の当たり外れ」と書いたのは根拠を超えていた。**

  さらに**「リトライでは通りません」と断じ、40 分後に通った。**
  疎通の実績は 7 回中 2 回。**恒久障害ではなく窓が断続的に開く状態**だった。
  ルーターの `!N` は実在したが、**そこから恒久性を結論したのが誤り。**

  ### `.pii-denylist` —— 再発見を発見として報告した

  この機体でローカルの pii-scan が `literal name detection off` で走っている件を
  「今日判明した」として報告したが、**`next-actions.md` の 154 行目に 2026-09-02 から
  書いてあった。**CI 側が secret から実体を作るので影響を受けない、という切り分けまで
  既に書かれていた。**冒頭だけ読んでボール表を挿入し、140 行目以降を読んでいなかった。**
  既存の記述のほうが正確で、**公開物の目視スキャンの手順** (CI 成果物を落として行う) まである。

  ### LP —— sshboard との相互紹介が公開された

  #258 がマージされ、en / ja 両ページで配信を確認 (`age: 0`・`x-cache: MISS`・
  アイコンが `200 image/png` 8,435 バイト)。**アイコンはリポに置いた**
  —— CSP が `img-src 'self'` (ADR-0047) なので外部から引くと何も出ない。

  **未解決**: 「完全自動」が *手順の自動化* か *無人運転* か。sshboard 側は
  「誰も見ていない状態でサーバに対してエージェントを走らせる口」を**作らないもの**として
  挙げているので、後者なら矛盾する。**sshboard 席に照会中** (#258 に申し送り済み)。

  ### 公開リポの Issue を連絡に使わない

  user 指示。**`meta-taro/zumen#16` は削除済み** (クローズでは公開のまま読めるため)。
  線引きは「公開リポかどうか」ではなく**「世界に読ませる価値があるか」** ——
  製品の不具合 Issue は公開されるべきもので、git-qa へ立てた 6 本は残す。

  **削除しても消えないものがある** (通知メール・検索キャッシュ・ミラー・webhook を
  拾った先)。dbboard では history 書き換え後も `refs/pull` に 676 件残った前例がある。
  **「削除した」と「読めなくなった」は別。**

- 日付: 2026-09-13〜23 (**v0.17.0 と v0.18.0 を出した。ゲート 1 は「再現しなかった」で決着。
  入口ページを骨格ごと書き直し、日本語ページと sshboard の相互紹介を足した。**

  ### v1.0 ゲート 1 (#161) — 直っていない。最初から起きていなかった

  0.17.0 の実物に本物のマウスで触って測った結果、**Aurora DSQL (IAM) で #161 は再現しない**。
  状態表示は `クエリ未実行` → `直前のクエリ` / `518 ms`・1 行と正しく遷移した。
  **「直した」ではなく「再現しなかった」**として、issue・ADR・roadmap の全部に同じ言葉で書いた。

  2 回目の押下 (ケース 3) は**わざと SKIP** した。`QueryPanel.svelte` の `execute` / `readPage` が
  `finally` で `busy` を落としているので、**フラグが立ちっぱなしになる仮説は静的に閉じている**。
  画面で確かめる価値が無いものを実行しても、記録が 1 行増えるだけ。

  **署名済みの証拠は `runs/20260915-185839`** —— operator `rays`・target `desktop/dbboard/0.17.0`・
  シート `#161 実行ボタンが押しても反応しない — 再現の確認（v1.0 ゲート 1）`。
  **本題と対照が `VERIFIED` (人の署名)、3 件目が意図した `SKIP`。**

  > **2026-09-24 訂正** —— ここには最初 `runs/20260916-192701`・6 件 VERIFIED と書いてあった。
  > **それは別のシート**（`desktop-0-17.tsv` = v0.17.0 の見た目検証）で、ゲート 1 ではない。
  > どちらも実在し、どちらも `rays` の署名入りなので、**間違いは「取れているか」ではなく
  > 「どれが証拠か」**だった。全 run を `sheet.path` で検索して特定した。
  >
  > **この誤りは保留の判断を歪めていた。**最優先の据え方は「人が署名した記録が残るまで
  > 本線を保留」で、その条件は **2026-09-15 に満たされていた**。
  > 参照先を間違えたまま「まだ取れていない」と扱い、git-qa#13 待ちだと考え、
  > さらに「判定キーを押す人が現地にいない」を新しいブロッカーとして
  > `next-actions.md` の 1 番に置いた。**どれも、条件が未達という前提の上に積まれていた。**
  >
  > **教訓は「run id を控える」ではない。**記録が実在することを確かめるだけでは足りず、
  > **それがどのシートの記録かを確かめる**必要があった。run ディレクトリは日付で並ぶので、
  > 近い日付の別シートと取り違えても、ディレクトリを見る限り何も矛盾しない。
  > `run.json` の `sheet.title` を読むまで区別が付かない。

  ### タグを間違った枝に打った — 0.15.0 が v0.18.0 として公開された

  `main` が 0.18.0 になっているか**確かめずに** `git tag v0.18.0` を渡した。#255 が未マージで、
  **0.15.0 の中身が v0.18.0 の名前で公開された**。release / tag を消し、#255 をマージし、打ち直した。

  user から「**やっていいこまんどしかしゅつりょくしないで**」。
  手順を渡す前に**その手順が前提としている状態を実測する**。これは docs に書く規則ではなく、
  渡す側の義務。

  ### rustls の勧告 — 新しい版のほうが脆かった

  RUSTSEC-2026-0285 を「`0.22.4` が原因」と誤診した。勧告は
  `unaffected = ["< 0.23.13"]` / `patched = [">= 0.23.45"]` で、**脆いのは新しい `0.23.40`** のほう。
  `cargo update -p rustls` は `0.23.43` で止まるので `--precise 0.23.45` が要る。
  **バージョンの大小と安全の向きは一致しない。**

  ### 初回描画の計測 (ADR-0156) — 数字を報告する先が無かった

  `StartupClock` を足して `report_first_paint` / `startup_timing` を生やしたが、
  **最初に書いた版は「報告するだけで誰も読み返せない」**形だった。
  さらに `docs/startup-measurement.md` に「MCP から読める」と**嘘を書いた**。user が見つけた。
  → About ダイアログに「最初の描画」を出して、**人が読める口**を付けた。

  **実測そのものは未了。**再起動直後の cold start が要るので同席が要る (user 側ボール)。

  ### ダウングレード (ADR-0155)

  「最新版が動かないとき前の版に戻す」の答えとして、**版を上げる前に設定を丸ごと写す**
  (`snapshot_before_version` → `connections.pre-<version>.toml`・バイト単位・create-new)。
  古い版が読めない設定で壊れるのを防ぐ。

  ### 入口ページを骨格ごと書き直した (#256 / #257 / #258)

  **h1 が製品名だと、いちばん大きい行が情報ゼロになる。**読み手はまだその名前を知らない。
  → `Turso, D1, Aurora DSQL — a different tool for every one of them.` に替え、
  「名乗り / 困りごと / どう消すか / 入手口 / 実物 1 枚」をスクロール前に収めた。

  **テストが全部緑のまま、ダウンロードボタンが画面外に落ちていた。**
  構造しか見ていなかったので見つからず、**実際に描画して初めて**分かった。
  hero を `max-height: min(400px, 42vh)` で抑え、カードをボタンに畳んだ。

  日本語ページ (`site/ja/`) を新設。**最初の版は素のまま公開された** — 各ページが自前の
  `<style>` を持つ構造だったため。→ **2 枚の `<style>` をバイト単位で比較するテスト**を足した。

  sshboard の相互紹介はカードにした (#258)。アイコンは**リポに入れた** —
  CSP が `img-src 'self'` (ADR-0047) なので、向こうのサイトから引くと何も出ない。

  ### git-qa 試験導入 — 判定キーを押す人が現地にいない

  **git-qa#13 (ライブビューが小さすぎて読めない) 待ち**という線引きは変わらないが、
  **新しい詰まりが見えた**: 判定キー (`d`/`f`/`a`/`s`) は**現地の git-qa ビューア窓で人が押す**もので、
  **user はリモートにいて触れない**。#13 が直っても、押す人がいなければ署名済み記録は取れない。
  → **「現地にいる日にまとめて署名する」前提で組み直す**必要がある (user 判断待ち)。

  git-qa#32 の検証は**一度無効だった** — 私のクリックもキー入力も WebView に届いておらず、
  「開かない」と報告したのを撤回した。**届いていないことに気づかずに出した報告**が最も危ない。

  ### Remote Control が切れている (dbboard 外・実測のみ)

  他セッションが「落ちた」と言われたので測ったら、**4 本とも生きていて当日応答していた**。
  切れていたのは **RC の接続だけ**。

  | 経路 | 状態 |
  |---|---|
  | RC 直接 | 切れている |
  | **同一マシンの生存セッション経由** | **通る** (warifu / zumen で実証) |
  | SSH で外から | **この mac mini は sshd 停止中**。有効化は持ち主の判断 |

  **切れた側は自分が切れたことに気づかない** (状態を知る公開口が無い)。実際 warifu は 9/22 に
  「席が消えた」のを「セッションが終わった」と誤診している。
  **エージェントから `/remote-control reconnect` は叩けない** (組み込みコマンドで Skill ではない)。
  → **中継は「届く経路」であって「直せる経路」ではない。**

  ### `gh` が特定 IP にだけ到達できない

  `github.com` の **`20.27.177.116` に到達できない時間帯**がある。失敗は**きっかり 4.01 秒**
  (接続タイムアウト)、`.113` を引けば 0.13 秒で通る。**回線ではなく宛先 IP の当たり外れ。**
  push が 1 回落ちた原因もこれ。**独立した 3 セッションが同じ IP で観測**している。
  詰まったら `dscacheutil -q host -a name github.com` で今の IP を見て、リトライで引き直す。


- 日付: 2026-09-09〜10 (**memory を救出した。v0.16.1 を出し、schema diff の残り半分（外部キー + インデックス）を入れた。**

  ### memory の救出 — 期限のあったボールが消えた

  旧 Windows 機にしか無かった memory を、user が 2 回に分けて zip で持ってきた。

  - **1 回目は違うものだった** — リポジトリ内の `.claude/`（issues / plans / status）で、
    ほとんど git に入っていた。`.claude` が**2 か所ある**のが紛らわしい:
    リポジトリの中（git 管理下）と**ホームの下（git の外・memory はこちら）**
  - **2 回目が本命** — `~/.claude` 全体、438 MB。**22 プロジェクト・memory 511 ファイル**

  dbboard 分 27 件を取り込み（`MEMORY.md` は両方の目次を継ぎ足して 27 行）、
  **§33 の健全性検査を通過**（目次に無い memory 0 / 実体の無い目次 0 / 28 ファイル・993 行）。
  記録が「リポに書けない情報の唯一の写し」と呼んでいた 2 件は**実在した**。

  **`~/claude-memory-from-win/`（3.2 MB）に memory だけ抽出**したので、438 MB の zip は削除可。
  **他 21 プロジェクト分はまだそこにしか無い**ので、この抽出フォルダは各リポで取り込むまで残す。
  **旧機は初期化してよい。**

  ### v0.16.1 と ADR-0150（パッチは枠を持たない）

  v0.16.0 が画面に出した欠陥 2 つ（タイトルバーのマーク / **Saved ボタンのスタイル欠落**）を
  0.16.1 で出した。**後者は私が v0.16.0 に入れたもの** — `class="chip"` と書いたが
  `.chip` は QueryPanel にあり、**Svelte のスコープ CSS は子に届かない**。
  `svelte-check` は型しか見ず、コンパイラは「要素の無いセレクタ」しか警告しないので
  **既存の検査では原理的に捕まらなかった** → `scoped-classes.test.ts` を追加。

  切ろうとしたら計画ドリフト検査に拒否された。**「見出し = 次の予約枠」規則により、
  UI 修正 2 件が "The half that was deferred" という名前で出る**ことになる —
  v0.15.0 が改名して逃げたのと同じ食い違い。原因は**「既に出荷したものの不具合は誰も予約していない」**
  ことで、roadmap の枠は構造上 minor 単位。→ **枠との照合は「枠を持つリリース」にだけ効かせる**
  （`releaseDue` の `bump === "patch"` を使うだけ）。

  ### schema diff の残り半分 — 保留は 1 つではなく 2 つだった

  ADR-0148 が「インデックスと制約」をひとまとめに保留していたが、調べたら**性質が全く違った**:

  | | 状況 | 結果 |
  |---|---|---|
  | **外部キー** | `foreign_keys` が ADR-0054 以来 trait にあり、**4 アダプタ実装済み**（turso 含む） | **何にも塞がれていなかった** → ADR-0151 |
  | **インデックス** | `table_ddl` の中にしか無く 6 中 3。trait + capability の追加 = **contract 層** | user が B+C を選択 → ADR-0152 |

  **判断の芯**:
  - **主キー自身のインデックスは全アダプタで除外** — 主キーは単独で比較済みなので二重報告になる
  - **一意制約のインデックスは残す** — 他に報告するものが無く、落とすと本物の差が消える
  - **インデックスは名前で、外部キーは列で照合** — 一貫性の欠如ではない。
    **インデックス名は人が書いたもの、外部キー名はエンジンが生成したもの**
  - **インデックス内の列順は差分** — 表の列順を差分にしない判断と正反対で、理由も正反対

  **contract に触ったので `dbboard-web` がミラー ADR を負う。**
  既存の検査 **3 つ**（wire 形 / `GET /capabilities` / ドリフト検査）が全部反応した = 設計どおり。

  ### `history.jsonl` は (c) だけ実施（ADR-0153）

  a（実装を戻す）/ b（スキーマ引退）は**両リポの ADR が要る**ので選ばず、
  **文書が嘘をつき続けるのを止める**ことだけやった。roadmap の `[x]` は
  「出荷され、その後 ADR-0089 で消えた」と読める形にし、**原文は残した**
  （書かれた当時は真だったので、消すと「かつて何が存在したか」が失われる）。

  ### AI がやれていないこと → user 側

  - **`dbboard-web` へのミラー ADR**（ADR-0152・contract 層）
  - **cold-start の計測** — roadmap が「実際の接続に対して」と条件を書いており、
    実店舗 DB に勝手に接続しないため**同席が要る**。v0.17 に残る唯一の実作業
  - 公開物の目視 PII スキャン（**v0.15.0 / v0.16.0 / v0.16.1 の 3 版**）
  - `main` への release PR / 他 21 プロジェクトの memory 取り込み)

- 日付: 2026-09-08〜09 (**v0.16.0 を出し、画面に出ていた欠陥 2 つで 0.16.1 も出した。UI の置き場所は user が 3 案から選んだ。**

  ### 出したもの

  | | |
  |---|---|
  | **v0.16.0** (2026-09-08) | ページング / JSON エクスポート / saved queries / schema diff / Dock 名称修正。資産 9 点・`latest.json` 0.16.0 |
  | **v0.16.1** (2026-09-09) | 「0.16.0 が画面に出したもの」— タイトルバーのマークと Saved ボタン。資産 9 点・`latest.json` 0.16.1 |

  PR #239 (Compare タブ) / #240 (欠陥 2 件 + 検査) / #241 (0.16.1 + 規則変更) はすべて CI 緑で merge。

  ### UI の置き場所は人が決めた (ADR-0149)

  ADR-0148 で「画面は人の判断」と保留していた分。**3 案を claude.ai のキャンバスに描いて出した**
  (Compare タブ / Structure 拡張 / 右クリック→ダイアログ)。返ってきた答えは
  **「A の形 + B の追従」**で、これは**そのままでは成立しない** —
  A の長所 (独立) と B の長所 (追従) が打ち消し合うため。

  噛み合わせ方: **サイドバーをフィルタではなく案内板にする。** 規則 3 つ —
  (1) 開いたときに現在の接続で埋める、そして手を離す (2) **画面にある報告は他所のクリックで
  差し替えない** (3) テーブルのクリックは「移動」であって「絞り込み」ではない。
  規則 2 は **effect の不在として実装**されているので ADR に明記した (不在のコードは
  後から誰かが足す種類のものだから)。

  ### 自分で入れた欠陥を、自分で出荷した

  **Saved ボタンにスタイルが当たっていなかった** (v0.16.0 で出荷)。`class="chip"` と書いたが
  `.chip` は QueryPanel の `<style>` にあり、**Svelte のスコープ CSS は子に届かない**。
  Windows 版で user が発見。`svelte-check` は型しか見ず、コンパイラは「要素の無いセレクタ」を
  警告するが「セレクタの無い要素」は警告しない — **既存の検査では原理的に捕まらなかった。**

  `scoped-classes.test.ts` を足した。判定は意図的に狭い:
  **「規則がアプリ内の別の場所には存在するのに、そこからは届かない」場合だけ**落とす。
  全走査でもう 1 件出たが偶然の同名 (Sidebar の `.conn` と ダイアログの `.conn`) で、
  理由を 1 行付けた例外にした (`deny.toml` と同じ形)。

  ### CI で 2 回落ちた。どちらも「手元では通る」形

  - **`@types/node` が `apps/desktop` の依存に無い** — 新しい検査が `node:fs` / `process` を
    使っていた。手元では解決し、CI の `--frozen-lockfile` では無い。
    **依存を足さず `import.meta.glob` に書き換えた** (Vite 自身の仕組み)。
    副産物: **vitest は既定で CSS を空文字にスタブする** (`css: true` で解消)。
  - **計画ドリフト検査** — v0.16.0 を切った時点で `[Unreleased]` の見出しが空になっていた。
    検査が正しかった方。

  ### パッチは枠を持たない (ADR-0150)

  0.16.1 を切ろうとしたら検査に拒否された。「`[Unreleased]` は次の予約枠の見出しを掲げる」
  規則により、**2 つの UI 修正が「The half that was deferred」という名前で出る**ことになる —
  **v0.15.0 が改名して逃げたのと同じ食い違い**。

  衝突する 2 つの規則はどちらも正しく、衝突の原因は
  **「既に出荷したものの不具合は、誰も予約していない」**こと。roadmap の枠は構造上 minor 単位
  (`v0.16.1` の行は「陳腐化した枠」と読まれる)。
  → **枠との照合は「枠を持つリリース」にだけ効かせる。**`releaseDue` が既に計算している
  `bump === "patch"` を使うだけ。パッチも見出しは要る / 陳腐化は今も報告 /
  **additive が 1 件入った瞬間に照合が戻る。**

  ### v0.17 の枠を置いた (中身は user 判断で差し替え可)

  release-cut が「roadmap がまだ v0.16 を予約している」と拒否したため。
  **新しく考えたものではなく、このセッションで書き留めて先送りしたもの 3 つ**:
  cold-start 計測 (Phase 5 唯一の未チェック) / `history.jsonl` の決着 (issue 0033) /
  schema diff のインデックス・制約 (ADR-0148 で保留)。

  ### やらかしと訂正

  - **`git add -A` が `.design/` (2.5 MB の生成物込み) を巻き込んだ。** 未 merge のうちに
    amend + force push で履歴から外し、`.gitignore` に追加 (873 KiB → 16 KiB)。
    生成物を履歴に残すと以後すべての clone が運ぶため。
  - ADR-0147 の記述 2 箇所が不正確だった (`history.jsonl` の書き手の所在、contract の記述)。
    push 前に訂正。

  ### AI がやれていないこと → user 側

  - **memory 27 ファイルの移送** — escalation #4 は **§28 の閾値 3 日 (09-09) を迎えても無反応**。
    申し送りどおりなら次は**催促ではなく自リポ起票** (§27 の正規経路は自リポ)
  - **公開物の目視 PII スキャン** — v0.15.0 / v0.16.0 / v0.16.1 の **3 版ぶん未実施**
  - `main` への release PR / v0.17 の枠の中身の可否)

- 日付: 2026-09-07 (**v0.16 の 3 項目を一日で。schema diff は画面だけ残した。PR #235 / #236 / #237 merge 済み。**

  ### 出したもの (すべて develop・CI 緑)

  | PR | 中身 | ADR |
  |---|---|---|
  | #235 | **JSON エクスポート** — CSV が潰す型を保つ | 0146 |
  | #236 | **saved queries** — 履歴の規則が及ばない場所に置く | 0147 |
  | #237 | **schema diff の比較エンジン** (UI 未着手) | 0148 |

  加えてこのセッションの最後に、schema diff の**配線** (`McpService::diff_schemas` +
  Tauri コマンド) を積んだ (未 push)。

  ### 判断の芯 (3 つとも「間違え方の向き」で決めた)

  - **JSON は CSV の句読点違いではない** (ADR-0146)。CSV は `exportValue` で全セルを
    文字列に潰す — `NULL`→空文字、document→JSON テキスト、blob→`<blob>`。
    表計算には正しく、機械が読み戻すには正しくない。**型が残ることが存在理由。**
    同名列 (`SELECT a.id, b.id`) は落とさず `id:2` にする — 黙って上書きすると
    **パースは通るのに列が 1 つ少ないファイル**になる。BOM は付けない (CSV は Excel の
    ために付けるが、JSON パーサはまさにそのバイトを拒否する)。
  - **saved queries は `localStorage` に置かない** (ADR-0147)。履歴は道具が勝手に記録する
    使い捨てで、`localStorage` はそれに合う。**意図して残すものを使い捨ての場所に置くのが、
    捨てられ方そのもの** — 「サイトデータを消去」で消え、config のバックアップに入らず、
    **別マシンへ移るとき付いてこない**。引っ越しで実地に学んだところ。
  - **差分は「同じだ」と言って間違える方が高い** (ADR-0148)。余計な差分は二度見で済むが、
    見逃しは移行を壊す。だから**型はエンジンが綴ったまま比較**(正規化なし)、
    列順は差分にしないが**主キーの順序は比較する**、片側にしか無いテーブルは 1 回だけ報告。

  ### 途中で判断を 1 つ覆した

  配線の初稿は「describe できないテーブルはその側から落として比較を続ける」だった。
  `DbError` に該当する種別が無く、探しているうちに**それ自体が ADR-0148 の避ける向きの
  間違い**だと分かった — 落としたテーブルは報告に「片側に無い」と出る。
  **答えられなかった質問を、確定した差分として提示することになる。**伝播させて失敗させる形にした。

  ### 見つけて記録したこと

  - **`history.jsonl` を書くコードが存在しない** (issue 0033)。reader/writer は egui クレート
    `dbboard-ui` にあり **ADR-0089 の引退で一緒に消えた**。`docs/roadmap.md` は今も
    `[x]` 完了で持ち、**`dbboard-web` は desktop が吐く前提の v:2 を実装済み**。
    (a) 実装を戻す / (b) スキーマを引退させる / (c) 予約済みと明記する。
    **(b) は両リポの ADR が要るので選んでいない。** v1.0 の contract 凍結前に決める。
  - issue 0029 の状態行が「実装はこれから」のままだった (出荷は 3 日前) → 訂正。
  - **nightly の `deps` は動いていた。外していたのは時刻の期待値** (cron 16:00 JST に対し
    実測 20:00〜21:00 JST)。疑う線を「翌 0:00 JST を跨いでも出ないとき」に引き直した。
  ...
  dbboard 2 / git-qa 0 と全プロジェクトほぼ空)。**本セッション時点で無反応。**
  §28 の閾値は 3 日 = 09-09。それを過ぎたら**催促ではなく自リポ起票**へ (§27 の正規経路は
  自リポで、#4 が拾われないとすればそこ)。

  ### 手元の回線が繰り返し落ちた

  `network is unreachable` / `Failed to connect to port 443` が **1 日で 6 回**。
  CI 監視ループが 2 回、エラーを「結果」と誤認して抜けた (エラー行に `pending` が
  無いため)。**エラーを読み飛ばすループに直した**。dbboard 側の問題ではない。

  ### AI がやれていないこと → user 側

  - **schema diff の UI 置き場所** — `CLAUDE.md` が「見た目は人が決める、AI が埋めた
    既定値は既成事実になる」と定めているので提案もしていない。**残りはこれだけ**
  - **v0.16 を切るか** — `release-due.mjs` は 4 件 = due。タグ push は人 (ADR-0121)
  - **memory 27 ファイルの移送** (#4 の返答待ちだが、ボールは user)
  - **`history.jsonl` の扱い** (issue 0033・web 側と両リポの ADR))

- 日付: 2026-09-06 (**二日空けての確認。日次 deps は動いていた — 外していたのは時刻の期待値。**

  ### 前回の申し送り 2 件は両方閉じた

  **日次 `deps` は走っている。** 09-04 17:30 JST 時点で `--event schedule` が 0 件
  だったのは事実だが、**その約 3 時間半後に初回が動いた**。二晩とも `deps` だけ
  success で他 3 ジョブは skipped = ADR-0144 の設計どおり
  (`33870383638` / `33962458025`)。

  **外していたのは時刻**。cron は `0 7 * * *` = 16:00 JST なのに、実測は
  **11:57Z と 11:07Z = 20:00〜21:00 JST**。GitHub は schedule を混雑時にずらす。
  「16:00 に出なければ設定を疑う」という申し送りは**毎日誤検知する**ので、
  疑う線を**翌 0:00 JST を跨いでも出ないとき**に引き直した。

  ローカルブランチ 2 本 (`fix/the-dock-shows-the-file-name` /
  `feature/the-hundred-and-first-row`) を削除。前者は #233 merge で remote が消えた
  残骸で、持っていた `db5cb91` は develop の `1e5a055` の amend 前の版。

  ### `dbboard-web` の ADR-0145 ミラーは後回し (user 判断)

  「ニーズが薄い」。Pacing Note では contract 変更のミラーは必須なので、
  **取り下げではなく延期**として `.claude/next-actions.md` に残した。

  ### 静かな二日間

  `0c2986b` 以降コミットなし、develop は origin と同期、作業ツリー clean。
  `release-due.mjs` は変わらず 2 件 = **切ってもよいが due ではない**
  (0.15.0 -> 0.16.0: Everyday work)。)
