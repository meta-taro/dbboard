...
設計上はもう 1 つ履歴がある — ADR-0017 の `history.jsonl`（スキーマは `dbboard-web` がミラー）。
**Tauri クライアントでこれを書いているコードは存在しない。** `dbboard-config` は今もパスを
解決し、contract も形を書いているが、クライアント再構築以降デスクトップの履歴は
webview のストレージだけ。**誰も書かないファイルに相乗りはできない。**
