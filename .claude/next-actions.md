...
| 7h | **ノート PC の自動更新が v0.10 → v0.11 で止まる** — ダウンロード後に落ちて上がってこず、手で起動すると v0.10 のまま。**未着手**。方針は決めてある: updater が終了する前に `on_before_exit` でパンくずファイルを置き、次の起動で「前回の更新が完了していません」+ 手動ダウンロード先を出す。0.12.0 の枠 | 実利用の摩擦 (2026-08-25) |
...
- 日付: 2026-08-25 その2 (**接続ダイアログの 2 件と、MCP からの接続登録**。
  ...
  **(3) 前提として先に直した本物のバグ** (ADR-0133 / `a4090af`)。`ConnectionAdmin` が
  ...
  `--no-verify` なし。**AI がやれていないこと**: 画面と実機での確認、それと 7h (ノート PC の
  自動更新) は未着手。→ **user 側 / 次セッション**。)
