...
| **公開された exe を目視で PII チェック** (v0.11.0 のぶん・未実施) | `.pii-denylist` はこのマシンに無い (untracked・§55)。機械では見られないので、実店舗名が exe に混ざっていないかは人の目でしか確認できない |
| **その次: #222 `feature/connection-order` を merge** — **画面確認は 2026-08-24 に実施済み**。出た指摘 4 件のうち、色セレクトと一覧の色は**あった** (欠陥ではない)、**絞り込み中の掴み手が握り拳カーソルになる件は修正した** (CSS の詳細度・`:not(:disabled)` で解決)。その後 2026-08-25 に目印まわりの UX (7d) を同じブランチに足したので、**merge 前にもう一度サイドバーを見る**: 接続行を右クリックして色見本が出るか / 選んだ色が行の左先頭に細い帯で出るか / 未設定の行と名前の位置が揃っているか **さらに 2026-08-25 に横区切り (7e / ADR-0131) を別ブランチ `feature/sidebar-panel-split` に積んだ** — こちらは #222 の上なので、#222 を merge した後に別 PR で出す。見るのは「区切り線を上下に動かせるか / W クリックで登録数なりの位置に戻るか」の 2 点 (§38 により催促はしない) | merge は §6 で user のもの。#219 は GitHub が閉じたので #222 で立て直した (ブランチも commit もそのまま) |
