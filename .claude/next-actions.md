...
| **v0.12** | 接続一覧を操れるようにする | 順番・検索・id でなく名前・色の目印 (#192)。前提の `ConnectionManager.svelte` 分割は済 (1,614 → 662 行)。**#192 の 3 条件は実装済** (名前表示 / ▲▼ 並び替え / 絞り込み)、色の目印だけ残り |
...
| 6 | 接続リスト D — 色マーク。パレット確定済 (2026-08-22)。**次はここから** | 0026 D |
...
| **その次: `feature/connection-order` を push → 接続一覧を 1 枚見る → merge** | 同上。**#216 に積んである**ので #216 が先。見るのは接続マネージャの一覧: ▲▼ が行の右端に収まっているか、絞り込み入力が下のテーブル検索と見分けが付くか、絞り込み中に ▲▼ が灰色になるか |
