...
- 日付: 2026-08-21 (**公開リポに実接続名が出ていたので全件掃除した。本文の置換は
  終わったが、GitHub の「編集履歴」に旧版が残っており、そこは画面からしか消せない。
  ここで詰まって終了。**
  ...
  **7 箇所ヒット** (issue #193 / #131 / #161 の本文、#142 と #161 のコメント、
  ...
  (`gh` にも REST にも無い)、**画面から 1 件ずつ消すしかない**。7 箇所ぶん残っている。
  **ここを消さない限り、今日の作業は表示が変わっただけ。**
  ③ **ブラウザから消そうとしたが権限で止まった。** #193 の `edited` は開けて履歴
  (meta-taro / dev-slot1-art) は見えるが、**削除のコントロールが出ない**。
  原因は権限で、`gh api repos/meta-taro/dbboard/collaborators/dokokade/permission`
  = `read` (admin/maintain/push すべて false)。**リビジョン削除には write が要る。**
  接続されている Chrome 拡張を 3 つとも調べたが、`dokokade` が 1 つと
  ログアウトが 2 つで、**`meta-taro` で入っている窓が 1 つも繋がっていなかった。**
  いただいたスクリーンショットの窓はリポジトリのタブに **Settings** が出ている
  (= オーナー表示 = meta-taro) ので実在はするが、その窓の拡張はこのセッションに
  繋がっていない。繋がっている窓を見分けられるよう、その窓を
  `https://github.com/login` に出したところで終了。**サインインは user 側**
  (エージェントは認証情報を入力しない)。
  ...
  **user 側ボール = ① `meta-taro` の窓でサインイン (ここが空くと 7 箇所の
  リビジョン削除に進める)、② 公開 `.exe` の PII 目視確認、③ history 書き換えの
  判断 (旧コミットにはまだ実名が乗っている。`git filter-repo` + force push で破壊的)、
  ④ v1.0 の残り 3 ゲート、⑤ §36 の改善要望、⑥ 従来からの継続分。**
  次のエージェント側タスク = **サインインが済んだら 7 箇所のリビジョン削除**。)
