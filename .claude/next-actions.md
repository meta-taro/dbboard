...
| # | やること | 出典 |
|---|---|---|
| 1 | #216 が develop に入る → **v0.11.0 を切る** = `node scripts/release-cut.mjs` + cargo check + commit。**ここまでエージェント。user は tag push だけ** (ADR-0121)。**切った直後に、#219 側の 4 つ — 並び替え / 絞り込み / 色 + タグの目印 / ドラッグ (ADR-0126・0127・0128) — の CHANGELOG を書く**。`release-cut.mjs` は `## [Unreleased]` の見出しをそのままバージョン見出しに書き換えるので、**切る時点で Unreleased にある行がそのまま v0.11.0 の中身になる**。4 つはまだ develop に無い (#219 の上) ので、先に書くと出荷していない機能を v0.11.0 が謳う。空になった `## [Unreleased]` が v0.12 の枠 | CHANGELOG 6 件 |
...
| やること | なぜエージェントには無理か |
|---|---|
| **いま: #216 を merge → #219 (#220 も) を merge** | push は §6。接続ダイアログ 6 枚 (追加/編集/エクスポート/インポート/複製/修復) と結果グリッド 4 枚 (本体/値ポップアップ/JSON ツリー/展開エディタ)。`svelte-check` は参照が解決することしか言わず、**CSS が当たっているかは言わない** |
| **その次: #219 `feature/connection-order` を merge (接続一覧を 1 枚見てから)** | **#216 に積んである**ので #216 が先。見るのは接続マネージャの一覧と接続フォーム: ▲▼ が行の右端に収まっているか、絞り込み入力が下のテーブル検索と見分けが付くか、絞り込み中に ▲▼ が灰色になるか、**色 + タグの目印がサイドバーと一覧の両方で名前を押しのけていないか**、**フォームの色セレクトの左端に色が出ているか** |
| ~~`feature/connection-drag-order`~~ → **#221 merged** (2026-08-24)。`feature/connection-order` に入り、CI green。**まだ人が見ていないのは 1 つだけ**: 絞り込み中に掴み手が灰色になって掴めないか (PR #221 のチェックリストで唯一未チェック)。#219 を見るときに一緒に | 見るのは #219 の 1 枚で足りる |
| **その次: #220 `chore/annotations-test-gaps` を merge** | **画面を見る必要は無い** (Rust のテスト 2 本だけ・非テスト行はゼロ)。`feature/connection-order` とは兄弟で、どちらも #216 の上に生えている。順不同 |
