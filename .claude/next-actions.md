...
| **v0.12** | 接続一覧を操れるようにする | 順番・検索・id でなく名前・色の目印 (#192)。前提の `ConnectionManager.svelte` 分割は済 (1,614 → 662 行)。**4 つとも実装済** (名前表示 / 掴み手で並び替え / 絞り込み / 色 + タグの目印)。**枠は空いた**ので CHANGELOG の `## [Unreleased]` に 5 件書いた (2026-08-25 に目印の直接編集と掴み手カーソルの修正を追加) (並び替えは ▲▼ が 1 リリースも出ないまま掴み手に置き換わったので、ドラッグと合わせて 1 件)。残りは #222 の merge だけ |
...
- **v0.11.0 は 2026-08-24 に切った。** いまは 3 件 = v0.12.0 が「出すべき」状態だが、
  中身 (#222) がまだ develop に入っていないので切れるのは merge の後。
...
| 7e | **サイドバーの横区切りを上下にドラッグ + W クリックで既定位置へ**。DB 接続一覧とテーブル一覧の境目。既定は接続の登録数で決める。**縦の splitter (ADR-0083 / `lib/layout/splitter.ts`) がそのまま雛形**: 数値の規則だけを純関数に切ってテストし、ポインタの配線はコンポーネント側に残す。**別ブランチ** — #222 は既に green で人も見た | 実利用の摩擦 (2026-08-24 依頼) |
...
| **その次: #222 `feature/connection-order` を merge** — **画面確認は 2026-08-24 に実施済み**。出た指摘 4 件のうち、色セレクトと一覧の色は**あった** (欠陥ではない)、**絞り込み中の掴み手が握り拳カーソルになる件は修正した** (CSS の詳細度・`:not(:disabled)` で解決)。その後 2026-08-25 に目印まわりの UX (7d) を同じブランチに足したので、**merge 前にもう一度サイドバーを見る**: 接続行を右クリックして色見本が出るか / 選んだ色が行の左先頭に細い帯で出るか / 未設定の行と名前の位置が揃っているか | merge は §6 で user のもの。#219 は GitHub が閉じたので #222 で立て直した (ブランチも commit もそのまま) |
