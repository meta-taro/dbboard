...
**期限のあるものは無い。ただし 1 と 2 は「最優先」の据え方そのものに関わる。**

| | ボール | 誰が | なぜ止まっているか |
|---|---|---|---|
| 1 | **git-qa の判定キーを押す** | **user** | **押せるのは現地の人だけ**。user はリモートで触れない。git-qa#13 (ライブビューが読めない) が直っても、**押す人がいなければ署名済み記録は取れない**。→ 「現地にいる日にまとめて署名する」前提で組み直すか、判断が要る |
...
- **`gh` が `github.com` の `20.27.177.116` にだけ到達できない**時間帯がある。
  失敗はきっかり 4.01 秒。**リトライで別 IP (`.113`) を引けば通る。**回線の問題ではない
