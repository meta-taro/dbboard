...
**残っている実装企画は 28 件**(+ バグ 2・保留 4・対象外 3)。内訳は下の表がそのまま全部。
数え漏らしを防ぐため、出典は roadmap の未チェック箱 / open issue / ローカル issue の
3 つだけに限り、それ以外の場所に企画を置かない。
...
**数えていないもの**: `.claude/issues/0025-database-workspace-expansion.md` の 8 フェーズ。
0025 自身が「1 人 + エージェントでは複数年、**日程ではなく選別フィルタとして持つ**」と
結論しており、roadmap には統合しないと決めてある。次の要望が来たとき「幹か枝か」を
判定するために使う文書で、着工対象の列には入れない。
