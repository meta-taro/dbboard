...
規則は `docs/roadmap.md` (ADR-0110) に前からある。「未リリースの中身が溜まったら出す」で、
日付では決めていない。見えていなかったのは**規則が無いからではなく、awk を叩かないと
数が見えなかったから**。→ `scripts/release-due.mjs` にして **push のたびに出る**ようにした。
...
- v1.0 だけは中身で決まらない (`docs/api-contract.md` の非互換変更 = major)。
  条件は `.claude/issues/0021-v1-0-criteria.md`。機能の数では上がらない。

### エージェント側 (上から順に)

| # | やること | 出典 | 状態 |
|---|---|---|---|
| 1 | #216 が develop に入る → **v0.11.0 を切る** (tag push は user) | CHANGELOG 5 件 | merge 待ち |
| 2 | 接続リスト C の 1 行分 — hover に id ではなく名前を出す | #192 / 0026 | 次 |
| 3 | 接続リスト A — ▲▼ で並び替え (`move_to`)。スキーマ変更不要 | #192 / 0026 | |
| 4 | 接続リスト B — リスト上の絞り込み入力 | #192 / 0026 | ここで #192 closed |
| 5 | `ConnectionManager.svelte` 1614 行を分割 (D を積む前に) | 0026 F | |
| 6 | 接続リスト D — 色マーク。パレット確定済 (2026-08-22) | 0026 D | 着手可 |
| 7 | Structure タブの描画を切り出し + 毎フレームの clone を落とす | 0016 | 借金 |
| 8 | MCP に export 動詞 — パスフレーズを線に載せない | #196 | |
| 9 | 手書きクエリの結果も、行が特定できるなら編集可に | 0022 | |
| 10 | 追加アダプター (順番は 0023 で確定済み) | 0023 | |
| 11 | 11 ロケールのうち 9 つが 9% (334 中 30 キー) | #181 | |
| 12 | 接続リスト E — グループ / フォルダ。**3 が効いたなら不要** | 0026 E | 条件付き |
| 13 | 競合ウォッチの仕組み | 0024 | v1.0 の後 |
...
| 検証シート 002 (MongoDB) → 001 (Firestore) | §22: 実施と合否は人間 |
...
| dbboard-web へ contract を写すか、写さないと明言するか | 相手リポの判断 |
