...
| **v0.12** | 接続一覧を操れるようにする | 順番・検索・id でなく名前・色の目印 (#192)。前提の `ConnectionManager.svelte` 分割は済 (1,614 → 662 行)。**4 つとも実装済** (名前表示 / ▲▼ 並び替え / 絞り込み / 色 + タグの目印)。残りは v0.11.0 を切って枠を空けること |
...
- ※ 2026-08-24 その2 (色 + 短いタグの目印) のエントリは、退避日 2026-08-24 その4 に
  `.claude/archive/next-actions-2026-08.md` へ全文退避。**まだ生きているのは画面確認だけ**で、
  それは上の「user 側でないと進まないもの」の表に残してある。

- ※ 2026-08-24 その3 の退避で、**その日のうちに 2 本目の日付エントリ**
  (▲▼ と絞り込み) も同じ場所へ移した。**まだ生きている決め事は 1 つだけで、
  それは上の「順番」の表の 1 番に書いてある**: v0.11.0 を切った*後*に
  ▲▼ と絞り込みの CHANGELOG を書く (先に書くと v0.11.0 の中身として下へ移る)。
