...
- **GitHub への到達が不安定。**2026-09-23 は `20.27.177.116` だけが落ちていて、
  リトライで `.113` を引けば通った。**2026-09-24 朝は GitHub の全 IP が落ち**、
  `api.anthropic.com` と `1.1.1.1` も落ち、`www.google.com` は 200 だが 7.2 秒 (通常の 50 倍)。
  **9/23 に「回線の問題ではない」と書いたのは根拠を超えていた。**IP ごとの当たり外れは
  実際に起きていたが、それは症状の 1 つで、回線側の劣化を否定する材料ではない。
