...
| **v0.12** | 接続一覧を操れるようにする | 順番・検索・id でなく名前・色の目印 (#192)。前提の `ConnectionManager.svelte` 分割は済 (1,614 → 662 行)。**4 つとも実装済** (名前表示 / 掴み手で並び替え / 絞り込み / 色 + タグの目印)。**枠は空いた**ので CHANGELOG の `## [Unreleased]` に 3 件書いた (並び替えは ▲▼ が 1 リリースも出ないまま掴み手に置き換わったので、ドラッグと合わせて 1 件)。残りは #222 の merge だけ |
