...
- **退避日 2026-08-24 その4**: 同じく 416 行でトリガを踏んだため、2026-08-24 その2
  (接続の目印 = 色 + 短いタグ) を追加で退避。ファイル末尾の `## 退避日 2026-08-24 その4` 以降がそれ。
...
- **退避日 2026-08-25**: サイドバーの横区切り (ADR-0131) を書き足して 454 行になり、
  400 行トリガを踏んだため、**2026-08-24 の その5 / その6 / その7** (並び替え / v0.11.0 を
  切った / v0.11.0 が出た) の日付エントリ 3 本と、**候補 A** (実利用摩擦の次テーマ)、
  **「接続名サニタイズ」節**を退避。見出し「退避日 2026-08-25」以降がそれ。
  候補 A は列挙していた 4 件のうち #192 が 7c / 7d で片付いており、残る 3 件は
  `next-actions.md` の「エージェント側」表に番号付きで載っている = **同じ話が 2 箇所**に
...
## 退避日 2026-08-24 その4 (2026-08-24 その2)

- 日付: 2026-08-24 その2 (**接続の目印 = 色 + 短いタグ**を実装。`4e0e6d9`、
  `feature/connection-order` の 3 commit 目。**計画 (0026 D) の 3 番目の条件を広げた**:
  「色だけでは目印にならない」の非色成分を、計画が並べていた 2 案 (短いタグ / アイコンの形) の
  うちタグにしたうえで、**タグを必須にした**。色名をテキストとして出す案は条件を字面どおり
  満たすが役に立たない — 接続の隣に「赤」と出ても、その接続の名前より情報が少ない。
  代わりに運用者が書く `prod` / `dev` が、色が立て替えていた意味そのものを運ぶ。
  **色だけのマークはフォームが保存を拒む**が、手で書いた config が持っていた場合は
  色名にフォールバックして**描く** (描かないと、ファイルには色があるのに行は無印に見える)。
  パレットは 8 色・`tokens.css` に 1 箇所定義で全テーマに値を持つ。タグは 12 文字まで。
  両方とも `mcp_alias` (ADR-0088) と同じ 3 状態 (None=据置 / Some(v)=設定 / Some("")=消去)。
  マークアップは `ConnectionMark.svelte` 1 つに閉じたので、**片方だけ描くことが構造的に不可能**。
  `mark_drift.rs` が Rust 側パレット / フロント側パレット / テーマトークン / 2 つのタグ長制限の
  4 つを突き合わせて、ずれたら落ちる。**テスト**: `marks.test.ts` 18 件、`mark_drift.rs` 4 件、
  Rust 側の正規化とエラー。`pnpm check` 314 files / 0 errors、`pnpm test` 568 passed (34 files)、
  pre-commit ゲート green。**ADR-0126** と DESIGN.md の identity colours 節を追加。
  **AI がやれていないこと**: 画面で見ること。→ **user 側: 接続フォームと一覧・サイドバー**。)

---

## 退避日 2026-08-24 その5 (2026-08-24 その4 と、完了した順番 2〜7)

- 日付: 2026-08-24 その4 (**順番 7 に着手して、対象が無いことが分かった**。`896a62e`、
  `chore/annotations-test-gaps` = #216 の上に生えた `feature/connection-order` の兄弟。
  issue 0016 の 5 項目のうち 4 つは **v0.4.0 で退役した egui クレート**宛てで、対応物が無い。
  残った `annotations.rs` のテスト穴 3 つのうち **実在した 2 つを埋めて 0016 は close**:
  手で壊された TOML だけが通る `Parse` の腕 (そこを空ストアとして読むと次の保存で
  全ノートが黙って消える) と、`prune` の retain 述語を個別に見るケース。
  **全文の理由は `.claude/issues/0016-structure-tab-render-refactor.md` が正本**
  — ここに写すと二重になるので書かない。**非テスト行は 1 行も触っていない。**
  4 か月生き残った理由は「振る舞い」ではなく**ファイルパスに対して起票した**ことで、
  同じ罠が **7b** にある (注記済み)。pre-commit ゲート green (ワークスペース全体)。
  **AI がやれていないこと**: 無し。画面確認も不要。→ **user 側: push と merge だけ**。)

完了して打ち消し線になっていた「順番」表の 6 行:

| # | やること | 出典 |
|---|---|---|
| ~~2~~ | ~~接続リスト C — hover に id ではなく名前を出す~~ → **完了** (`b48c974`、#216 の中) | #192 / 0026 |
| ~~3~~ | ~~接続リスト A — ▲▼ で並び替え (`move_to`)~~ → **完了** (`917aa09`、`feature/connection-order`) | #192 / 0026 |
| ~~4~~ | ~~接続リスト B — リスト上の絞り込み入力~~ → **完了** (同ブランチ)。**#192 の 3 条件は揃った**が、close は develop に入ってから | #192 / 0026 |
| ~~5~~ | ~~`ConnectionManager.svelte` 1614 行を分割~~ → **完了** (1,617 → 662 行、#216 の中)。ついでに `ResultGrid.svelte` も 1,132 → 553 行 (**Svelte 側の**ハードリミット超過は 0 件になった。Rust 側は 7b が残っている) | 0026 F |
| ~~6~~ | ~~接続リスト D — 色マーク~~ → **完了** (`4e0e6d9`、`feature/connection-order`)。色**と**短いタグの 2 枚組にした (ADR-0126) | 0026 D |
| ~~7~~ | ~~Structure タブの描画を切り出し + 毎フレームの clone を落とす~~ → **完了扱い / 中身は消滅** (`896a62e`、`chore/annotations-test-gaps`)。0016 の 5 項目のうち **4 つは egui クライアント (`crates/dbboard-ui`) 宛て**で、そのクレートは v0.4.0 で退役済 (`af17200` / ADR-0089)。Structure タブは今 `StructurePanel.svelte` (468 行) で、毎フレーム再描画そのものが無い。残った 1 項目 (`annotations.rs` のテスト穴) のうち実在した 2 つを埋めて 0016 は close | 0016 |

---

## 退避日 2026-08-25

### 最終更新 — 2026-08-24 その7 / その6 / その5
...
- 日付: 2026-08-24 その6 (**v0.11.0 を切った**。user の「切って」を受けて
  ...
  ADR-0121 のとおり **判断と タグ push は user**、機械的な半分だけをこちらで済ませた。
  **`release-plan.test.mjs` が落ちた**: 「`docs/roadmap.md` がまだ v0.11 の枠を予約している
  のに 0.11.0 は出ている」。枠表から行を消すのは**この表ができて以来はじめて**で
  (`git log -S` で `6bea6bb` 以降に前例なし)、消し方が決まっていなかった。決めたのは
  「**出した瞬間に枠表から消える**」。出した後は CHANGELOG がその問いに答えるので、
  同じ問いに答えが 2 つあると、古い方＝計画の方が嘘をつき始める。roadmap 側にもその 1 文を書いた。
  **切った直後に v0.12 の CHANGELOG を書いた** (「順番」1 番。user から
  「これって、そっちでやることじゃないの？」と言われた分)。3 件 — 並び替え / 絞り込み /
  色 + タグの目印。**▲▼ は 1 件も書いていない**: 追加も撤去も同じ未リリース区間の中で
  起きた (ADR-0128) ので、両方書くと**どの利用者も通っていない状態**を 2 行かけて説明する
  ことになる。CHANGELOG が答えるのは「前のリリースから何が変わったか」であって、
  開発中に何があったかではない。**置き場所はブランチ側** (`feature/connection-order` = #222)。
  develop に書くと、develop に無い機能を develop の CHANGELOG が謳う。そのために develop を
  先にこのブランチへ merge して、`## [Unreleased]` の衝突をいま解いてある。
  **AI がやれていないこと**: push と タグ push。→ **user 側**。)
...
- 日付: 2026-08-24 その5 (**ドラッグでの並び替え**。`feature/connection-drag-order`
  ...
  **見積もっていた 3 つのコストのうち 2 つは既に払い終えていた**: キーボード等価物は
  ...
  `svelte-check` 0 error / vitest 578 pass / release ビルド green。

  **画面で見てもらって 2 つ直した (ADR-0128)**: (1) **▲▼ を撤去**。ドラッグが効いた時点で
  1 行に 5 つのボタンが並んでいて、うち 2 つは掴み手と同じことを言っていた。キーボードは
  掴み手に移し、フォーカス中の `↑↓` で `move_to` を呼ぶ (掴み手はフォーカスでも見える)。
  (2) **名前の左端がガタガタだった**のを直した。行の子が 3 つになった時点で
  `space-between` が余白を真ん中の子の両側に割り振るようになり、**名前の長さで
  インデントが決まって**いた。`.row-main` に `flex: 1`。**どちらも型検査にも単体テストにも
  映らない**種類の壊れ方で、画面を見ないと出てこない。
  **AI がやれていないこと**: 直した後の見た目の確認。→ **user 側: もう一度見て、push**。)
...
### 「user 側でないと進まないもの」表から落とした 5 行

4 行は 2026-08-24 に完了したもの、1 行 (`公開 exe の目視 PII チェック`) は
同じ表に 2 度書かれていた重複。表側には 1 行にまとめた案内を残してある。
...
| ~~#224 `release: v0.11.0` を merge~~ → **merged** (2026-08-24 / `b99e3ec`)。`main` = `develop` の中身に一致、`v0.11.0` は `main` の祖先 | merge は §6 で user のもの。**今回はタグが先に出ていた**ので、この PR で公開されるものは無く `main` を追いつかせるだけだった |
| ~~`feature/connection-order` を push~~ → **完了** (`19754e3`) | #222 に v0.12 の CHANGELOG (`6f4df88`) が乗った |
...
### 「参考: 配布済 exe の使用シグナル確認 / 再配布」から縮めた 2 段落

どちらも「以前ここに書いてあった誤り」への訂正で、訂正先の本文はもう無い。
節そのものは残してあり、生きている事実 (publish ジョブが release オブジェクトを
自力で用意する / バンドルは CI が作る) は 2 行に縮めて置いてある。
...
## 退避日 2026-08-25 その2 — 最終更新ログ 2026-08-24 その8 〜 2026-08-25

`.claude/next-actions.md` が 429 行に達したため退避した (baseline §31)。
新しい順 (元ファイルと同じ並び)。

- 日付: 2026-08-25 (**サイドバーの横区切りを動かせるようにした**・ADR-0131。
  user の「左の DB 接続とテーブルの区切りの横ライン、上下に移動できて W クリックで定位置」。
  **別ブランチ** `feature/sidebar-panel-split` = `feature/connection-order` の上
  (#222 はもう green で人も見たので、同じ枠をもう一度見てもらわない)。
  縦の splitter (ADR-0083) と同じ分担 — 数値の規則は `lib/layout/panel-split.ts` の
  純関数 (テスト 19 本)、ポインタの配線だけ `Sidebar.svelte`。
  **「登録数で状況変わります」への答えが設計の本体**: 保存する値を `number | null` にして、
  **null = 一度も掴んでいない** を意味させた。null の間は接続の登録数から高さが決まるので、
  接続を足すたびに勝手に広がる。W クリックは定数を書き戻すのではなく **null に戻す** —
  行数が固定でないものに「定位置」があるとすれば、それは数値ではなく規則の方だから。
  **ついでに前からあったバグが消えた**: 接続一覧に高さも `overflow` も無く、数が増えると
  テーブル一覧が窓の下へ押し出されていた。 `pnpm check` 0 error / vitest **597 pass** (+19) / pre-commit ゲート green。
  **AI がやれていないこと**: 画面で見ること。→ **user 側 (見たければ・§38 により催促はしない)**。)

- 日付: 2026-08-24 その8 (**v0.11.0 が `main` に着地した**。#224 `release: v0.11.0`
  (`develop` → `main`・50 commits) を作り、user が merge。`origin/main` = `b99e3ec`、
  中身は `develop` (`ecc51dd`) と完全一致、`v0.11.0` は `main` の祖先。
  **今回だけ順序が逆だった**: 前回まで (#211 / #197 / #173 / #158 / #155) は
  「PR を merge → タグを push」だったが、今回は先にタグを打って公開まで済んでいたので、
  この PR で新しく公開されるものは無く `main` を追いつかせるだけだった。本文にそう明記してある。
  **タグの後に 2 件乗っている** (`866eff1` / `ecc51dd`・`.claude/tools/` の道具カード削除のみ・
  計 606 行削除でコードは 0 行)。`main` の先端がタグちょうどでなくなるが、**別ブランチを切らずに
  `develop` → `main` のまま出した** — `CLAUDE.md` が `main` に求めているのは「直接 commit しない」
  であって「先端がタグと一致すること」ではない。過去 5 回も全部 `headRefName: develop`。
  前回たまたまタグちょうどだったのは develop が動いていなかっただけで、規約ではない。
  **AI がやれていないこと**: #222 / #223 の merge、公開 exe の目視 PII チェック。→ **user 側**。)

- ※ 2026-08-24 の その5 / その6 / その7 (並び替え / v0.11.0 を切った / v0.11.0 が出た) は
  `.claude/archive/next-actions-2026-08.md` へ全文退避 (退避日 2026-08-25)。**そこから
  生きているのは 2 つだけ**: (1) `!` コマンドの 2 分上限に pre-push の release ビルドが
  収まらないので、**push は普通の窓を 1 枚開けて渡す** (上限があるのは Claude Code 側で、
  フックでもマシンでもない)。(2) 出したバージョンの枠は roadmap の枠表から消す
  — 出した後は CHANGELOG がその問いに答えるので、答えが 2 つあると計画の方が嘘をつき始める。
...
- ※ 2026-08-24 の その2 (色 + タグの目印) と その3 (▲▼ と絞り込み) のエントリは
  ...
  v0.11.0 を切った*後*に並び替え / 絞り込みの CHANGELOG を書くこと。
  **後者も 2026-08-24 に完了** (「順番」1 番)。先に書くと v0.11.0 の中身として下へ移るので、
  順序の方が本体だった。
